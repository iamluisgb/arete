#!/usr/bin/env node
// Prepara dist/ para Cloudflare Pages. Mismo esquema que bookreader.
//
// Dos reglas:
//
//   1. LISTA BLANCA de lo publicable. El repo contiene tests, evals, tools,
//      prototipos y notas de trabajo que no pintan nada en producción. Copiar la
//      raíz entera publica todo eso sin que nadie lo decida.
//
//   2. El origen es el COMMIT, no el disco. Se extrae HEAD con `git archive` y se
//      construye de ahí: lo desplegado es exactamente un commit, y lo que tengas
//      sin commitear no puede colarse.
//
...
if (fromWorktree) {
  ...
  tmp = await mkdtemp(join(tmpdir(), 'arete-build-'));
